    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
      ...
      {loading ? (
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 text-center space-y-1">
          ...
          {rows.length === 0 && (
            <p className="text-xs text-slate-500">Add one to start enforcing the §(g) gate at permit-sign time.</p>
          )}
        </div>
      ) : (
            ...
            return (
              <li key={row.id} className="px-4 py-3 flex items-center gap-3 flex-wrap">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold text-slate-900">{row.worker_name}</p>
                  <p className="text-[11px] text-slate-500">
                    {TRAINING_ROLE_LABELS[row.role]} · completed {row.completed_at}
                    {row.expires_at && (
                      <> · {expired ? <span className="text-rose-700 font-semibold">expired {row.expires_at}</span> : <>expires {row.expires_at}</>}</>
                    )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40 overflow-y-auto py-10">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-5 space-y-4">
        <header className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-slate-900">Add training record</h2>
