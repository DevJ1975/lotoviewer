...
// Federal OSHA 29 CFR Part 1910 (General Industry) is no longer ingested through
// this single-markdown manifest. It now goes through scripts/osha_1910_ingest.py,
// which parses the eCFR XML into ONE knowledge_documents row PER section/appendix
// with pinpoint `metadata.section` citations (a single mega-document here could
// only ever cite "[29 CFR Part 1910]"). The partial OCR'd HazCom seed it
// superseded was removed with it.
//
// This manifest remains for any other ship-with-the-product regulation markdown
// (e.g. a future CalOSHA crawl). Add an entry + drop the file in apps/web/seed/.
  ...
  for (const entry of MANIFEST) {
    try {
      ...
      try {
        ...
        if (code === 'ENOENT') {
          // Manifest entry for a file that hasn't been generated yet
          // (e.g. the OSHA 1910 master, which is built locally via
          // scripts/ingest-osha-1910.mjs). Skip silently rather than
          // failing the whole batch — operator runs the generator,
          // then re-POSTs this endpoint.
          report.push({ key: entry.key, action: 'error', error: 'seed file not found (run the generator script first)' })
          errored++
          continue
        }
        throw e
