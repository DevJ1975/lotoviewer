...
//
// 'revoke'     — marks the row revoked. The public URL stops working;
//                no new row is created. Tenant has no review surface
//                until the admin generates a fresh link.
// 'regenerate' — revokes the existing row AND mints a new tenant-wide
//                public row, cycling the token. Use when the link has
//                been over-shared and you want a new URL that
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Step 2 (regenerate only): mint a fresh tenant-wide public row,
  // cycling the token. No equipment snapshot — the reviewer page
  // queries live, so there's no stale data to copy forward.
