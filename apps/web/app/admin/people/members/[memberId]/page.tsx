...
export default function AdminMemberDetailPage() {
  ...
  const onConfirmMerge = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/admin/people/members/${mergeTargetId}`)
    } catch (err) {
      if (mountedRef.current) {
  ...
  if (!member) {
    return (
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <Link href="/admin/people/members" className="inline-flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-brand-navy">
              ...
              <div className="rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-900 dark:bg-amber-950/40 dark:text-amber-100">
                ...
                {member.profile_id && mergeTarget.profile_id && (
                  <p className="mt-1 font-semibold text-rose-700 dark:text-rose-300">
                    Both members have login. Revoke one via /admin/people/users first.
                  </p>
                )}
              </div>
            )}
