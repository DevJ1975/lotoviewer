...
// /admin/prop65 — Compliance dashboard.
//
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            <Link href="/admin/prop65/annual-review" className="hover:underline">Record review →</Link>
          </p>
        </div>
        <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
          <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            <Link href="/admin/prop65/sites" className="hover:underline">Manage →</Link>
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800">
          ...
          <div className="flex items-center gap-3 text-xs">
            <Link href="/admin/prop65/chemicals" className="text-brand-navy hover:underline">Chemicals</Link>
            <Link href="/admin/prop65/sites" className="inline-flex items-center gap-1 text-brand-navy hover:underline">
        ...
        {rows === null ? (
          ...
          <p className="px-4 py-6 text-sm text-slate-500 dark:text-slate-400">No California sites yet. <Link href="/admin/prop65/sites" className="text-brand-navy hover:underline">Add one</Link> to begin.</p>
        ) : (
              ...
              {rows.map(r => (
                <tr key={r.site_id}>
                  <td className="px-4 py-2">
                    <Link href={`/admin/prop65/sites/${r.site_id}`} className="text-brand-navy hover:underline font-medium">{r.site_name}</Link>
                  ...
                  <td className="px-4 py-2 text-right">
                    <Link href={`/admin/prop65/sites/${r.site_id}`} className="text-[11px] text-brand-navy hover:underline">Open →</Link>
                  </td>
                </tr>
              ))}
            </tbody>
