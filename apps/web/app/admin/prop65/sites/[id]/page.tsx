...
// /admin/prop65/sites/[id] — Per-site detail with three tabs.
...
export default function Prop65SiteDetailPage({ params }: PageProps) {
  ...
  if (!site) return <div className="max-w-4xl mx-auto px-4 py-6"><Link href="/admin/prop65/sites" className="text-xs text-brand-navy hover:underline">← Back to sites</Link><p className="text-sm text-slate-500 mt-4">Site not found.</p></div>

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      <div>
        <Link href="/admin/prop65/sites" className="inline-flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300">
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-200 dark:border-slate-700">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Link href={`/admin/prop65/assessments/new?siteId=${site.id}`} className="text-xs text-brand-navy hover:underline">+ Assessment</Link>
          <Link href={`/admin/prop65/warnings/new?siteId=${site.id}`} className="text-xs text-brand-navy hover:underline">+ Warning</Link>
        </div>
      </div>
