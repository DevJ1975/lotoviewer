...
import { ISO14001_CLAUSE_MAP } from '@soteria/core/iso14001'

// /admin/evidence/iso14001 — ISO 14001:2015 clause map landing.
//
// Lists every clause we satisfy with the environmental modules that
// contribute evidence. Each row drills into
// /admin/evidence/iso14001/[clauseCode] which shows curated pins.

interface ClauseEvidenceRow {
  clause_code: string
}

export default function Iso14001MapPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      for (const row of ((data ?? []) as ClauseEvidenceRow[])) {
              ...
              {ISO14001_CLAUSE_MAP.map(entry => (
                <tr key={entry.code} className="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                  <td className="px-4 py-2 font-mono text-xs text-slate-900 dark:text-slate-100">
                    <Link href={`/admin/evidence/iso14001/${encodeURIComponent(entry.code)}`} className="text-brand-navy hover:underline">
                      {entry.code}
                    </Link>
                  </td>
