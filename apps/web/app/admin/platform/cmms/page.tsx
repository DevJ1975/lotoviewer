...
// /admin/platform/cmms — list + add CMMS integrations. Each row drills into
// /admin/platform/cmms/[id] for detail (recent events, retry button).
        ...
        <ul className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl divide-y divide-slate-100 dark:divide-slate-800 overflow-hidden">
          {rows.map(r => (
            <li key={r.id} className="px-4 py-3">
              <Link href={`/admin/platform/cmms/${r.id}`} className="block hover:bg-slate-50 dark:hover:bg-slate-900/40 -mx-4 -my-3 px-4 py-3 transition-colors">
