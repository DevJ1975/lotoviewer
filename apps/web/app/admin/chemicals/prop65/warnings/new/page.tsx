...
function NewWarningForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/admin/chemicals/prop65/sites/${siteId}`)
    } catch (e) {
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      ...
      <form onSubmit={submit} className="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 space-y-4">
        ...
        <fieldset className="text-xs font-medium text-slate-700 dark:text-slate-300">
          ...
          {options.length === 0 ? (
            <p className="text-slate-500 dark:text-slate-400 text-xs">No confirmed links yet. Visit <Link href="/admin/chemicals/prop65/chemicals" className="text-brand-navy hover:underline">Chemicals</Link> to confirm.</p>
          ) : (
