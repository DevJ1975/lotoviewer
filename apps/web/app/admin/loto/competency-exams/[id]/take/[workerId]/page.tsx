...
// /admin/loto/competency-exams/[id]/take/[workerId] — proctored exam.
//
...
function TakeExam() {
  ...
  async function submit() {
    ...
    try {
      ...
      // the /admin/loto/training-records page if needed.
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      <div>
        <Link href={`/admin/loto/competency-exams/${examId}`} className="inline-flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300">
      ...
      {result ? (
        <section className="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 text-center space-y-2">
          ...
          <button
            type="button"
            onClick={() => router.push(`/admin/loto/competency-exams/${examId}`)}
