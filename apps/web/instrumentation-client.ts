...
if (dsn) {
  Sentry.init({
    ...
    // Scrub sensitive fields out of every event before it ships — auth
    // headers, signature payloads, API keys, tenant headers, and the
    // credential-bearing URLs (invite tokens in the query string, Supabase
    // recovery tokens in the fragment). See lib/security/scrubEvent.ts.
    beforeSend(event) {
