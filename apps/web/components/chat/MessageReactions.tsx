...
import { addReaction, removeReaction, type ChatReactionAggregate } from '@/lib/chat/client'

// Slack-style reaction strip with a quick-add picker. Tapping an
// existing pill toggles your own reaction; the picker contributes a
// fixed shortlist (matches what the most-used emojis are in the
// existing safety-engineering vocabulary).
...
interface Props {
  ...
  onChange?:  (next: ChatReactionAggregate[]) => void
}
...
export default function MessageReactions({ messageId, reactions, onChange }: Props) {
  ...
  async function toggle(emoji: string) {
    ...
    try {
      if (has) await removeReaction(tenant.id, messageId, emoji)
      else     await addReaction(tenant.id, messageId, emoji)
    } catch {
  ...
  return (
    <div className="mt-1 flex flex-wrap items-center gap-1">
      {reactions.map(r => {
        const isMine = !!mine && r.user_ids.includes(mine)
        return (
          <button
            type="button"
            key={r.emoji}
