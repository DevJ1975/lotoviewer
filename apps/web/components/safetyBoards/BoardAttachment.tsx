...
import { getBoardAttachmentUrl, type ThreadAttachment } from '@/lib/safetyBoards/client'

// Mirrors components/chat/ChatAttachment.tsx but talks to the
// safety-boards bucket. Kept separate so the access-control story
// (chat-channel membership vs board-tenant membership) stays clear.

export default function BoardAttachmentView({ attachment }: { attachment: ThreadAttachment }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const u = await getBoardAttachmentUrl(tenant.id, attachment.id)
  ...
  if (error) return <div className="text-xs text-rose-700 dark:text-rose-300">Failed to load: {error}</div>
  if (!url)  return (
