-- Migration 292: read-path consolidation RPCs (perf PR 2).
--
...
as $$
  ...
  -- invite_cancelled_at is part of the membership predicate, not an extra
  -- check the caller can forget. Migration 190 puts it inside
  -- current_user_tenant_ids(), and the gate enforces it too; an RPC that
  -- consolidates the gate's reads has to carry it or a revoked member
  -- comes back with a live role.
