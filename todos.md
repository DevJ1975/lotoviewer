...
- [x] ~~**Test-suite health (dev task, not a blocker):** 81 pre-existing unit tests across 14 files fail on `main`~~ — **resolved, and the note was stale.** Measured on `main` 2026-08-17: the full suite is **329 files / 4092 tests / 0 failures** and `tsc --noEmit` is clean for `apps/web`. The failures were repaired at some point without either record being updated, and the two records disagreed anyway (this said 81; `.github/workflows/repo-health.yml` said 127). CI now gates the whole suite plus typecheck — see the `verify` job.
- [ ] **Lint debt (dev task, the real remaining gap):** `eslint .` reports **82 errors + 70 warnings**. It runs in CI as a non-blocking step; flip `continue-on-error: false` in the PR that clears it. Breakdown: `react-hooks/preserve-manual-memoization` 26, `react/no-unescaped-entities` 20, `react-hooks/refs` 19, `react-hooks/purity` 10, `prefer-const` 3, `react-hooks/globals` 2, `@typescript-eslint/no-this-alias` 1, `react-hooks/static-components` 1. The ~24 in the last four rules are mechanical; the 55 React Compiler ones each need the component read.
- [ ] **`packages/core` is not typechecked.** `tsc --noEmit -p packages/core/tsconfig.json` fails with 9 errors across 4 files, two of them compiler-option failures (`TS2550`: `.at()` needs `lib` ≥ ES2022 — `packages/core/src/forecast.ts:46,168`). The CI `typecheck` gate is scoped to `apps/web` until this is fixed.
- [ ] **`services/sds-parser` (Python) is absent from CI.** It has a real suite at `services/sds-parser/tests/test_parser.py` plus `scripts/test_osha_1910_ingest.py`; nothing runs either.
